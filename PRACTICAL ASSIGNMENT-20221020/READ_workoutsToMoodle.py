...
def read_file_to_df(filename):
    ...
    if list(data[j].keys())[0] == 'points':
        try:
            start = \
            list(list(list(data[data.size - 1].items()))[0][1][0][0].items())[
                0][1][0]
...
df_res.loc[NaN_index[0], "calories_kcal"] = np.mean(calories_burned_rate) * \
                                            df_res.loc[
                                                NaN_index[0], "duration_s"]
...
DF1["duration_s"] = (DF1["duration_s"] - DF1["duration_s"].min()) / (
            DF1["duration_s"].max() - DF1["duration_s"].min())
DF1["distance_km"] = (DF1["distance_km"] - DF1["distance_km"].min()) / (
            DF1["distance_km"].max() - DF1["distance_km"].min())
DF1["calories_kcal"] = (DF1["calories_kcal"] - DF1["calories_kcal"].min()) / (
            DF1["calories_kcal"].max() - DF1["calories_kcal"].min())
DF1["speed_avg_kmh"] = (DF1["speed_avg_kmh"] - DF1["speed_avg_kmh"].min()) / (
            DF1["speed_avg_kmh"].max() - DF1["speed_avg_kmh"].min())
DF1["created_date_num"] = (DF1["created_date_num"] - DF1[
    "created_date_num"].min()) / (DF1["created_date_num"].max() - DF1[
    "created_date_num"].min())
DF1["start_timee_num"] = (DF1["start_timee_num"] - DF1[
    "start_timee_num"].min()) / (DF1["start_timee_num"].max() - DF1[
    "start_timee_num"].min())
DF1["end_time_num"] = (DF1["end_time_num"] - DF1["end_time_num"].min()) / (
            DF1["end_time_num"].max() - DF1["end_time_num"].min())
...
X_train, X_test, y_train, y_test = train_test_split(DF1_data, DF1_class,
                                                    random_state=42,
                                                    test_size=0.3)
